...
def eval(args):
    ...
    loss, acc, py_reader, feed_datas, _ = network.network(items_num, args.hidden_size, args.step, args.batch_size)
    ...
    try:
        ...
        py_reader.set_sample_list_generator(test_data.reader(batch_size, batch_size * 50, True))
        py_reader.start()
        try:
            while True:
            ...
            py_reader.reset()
        logger.info("TEST -->Recall@50: %.4lf" %
                    (acc_sum / count))
    except ValueError as e:
        logger.info("eval --> error: there is no model in " + model_path)
